...
const PublicLoginPage = () => {
  ...
  const onSubmit = async ( data ) => {
    try {
      ...
      const result = await sdk.login( data.email, data.password, "public");
      if ( !result.error ) {
  ...
  return (
      ...
      <div className="min-h-full flex justify-center w-full max-h-full h-full">
        <section className="md:w-1/2 w-full flex flex-col items-center justify-center bg-white">
          <form onSubmit={ handleSubmit( onSubmit ) } className="flex flex-col max-w-md w-full px-6 mt-[9.375rem]">
            ...
            <Link className={ `self-end mb-6 font-semibold text-sm bg-clip-text bg-gradient-to-l from-[#33d4b7_9.11 %] to-[#0d9895_69.45 %] text-transparent` } to="/public/forgot">Forgot Password</Link>
    ...
    <h3 className="text-center text-sm text-gray-800 normal-case">Don't have an account? <Link
      className="self-end mb-8 my-text-gradient font-semibold text-sm" to="/public/sign-up">Sign
      up</Link> </h3>
  </div>
          </div>
