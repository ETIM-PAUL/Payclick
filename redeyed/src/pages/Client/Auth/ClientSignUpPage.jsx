...
const ClientSignUpPage = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const result = await sdk.register(data.email, data.password, "client");
      if (!result.error) {
        dispatch({
          type: "LOGIN",
          payload: result,
          ...
          <div className="oauth flex flex-col gap-4 max-w-md w-full px-6 text-[#344054] grow">
            <div>
              <h3 className="mt-5 text-center text-sm text-gray-800 normal-case">Already have an account? <Link
                className="self-end mb-8 my-text-gradient font-semibold text-sm" to="/client/login">Login </Link> </h3>
            </div>
          </div>
